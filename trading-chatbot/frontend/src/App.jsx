...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex-1 flex items-center justify-center bg-[#0b0e11] text-gray-400">
...
function App() {
  ...
  // Fetch chart history when a ticker is selected
  useEffect(() => {
    if (selectedTicker) {
  ...
  // Clear chat history when switching tickers
  useEffect(() => {
    setChatHistory([])
  }, [selectedTicker])
  ...
  const handleChat = async (e) => {
    ...
    try {
      ...
      console.error(err)
      setChatHistory(prev => [...prev, { role: 'error', content: 'Failed to get response. Please try again.' }])
    } finally {
      setLoading(false)
    }
  }
          ...
          {selectedTicker ? (
            <>
              ...
              <div className="flex-1 p-4 flex flex-col gap-4 overflow-hidden">
                {/* Fundamentals Row */}
                <FundamentalsComponent ticker={selectedTicker} />

                {/* Main Split: Chart & Chat */}
                ...
                <div className="w-96 bg-[#151924] rounded-xl border border-gray-800 flex flex-col shadow-xl h-full">
                  ...
                  <div className="flex-shrink-0 border-b border-gray-800 bg-[#1a1f2e]">
                     <SentimentComponent ticker={selectedTicker} />
                  </div>
                      ...
                      <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed ${
                          msg.role === 'user' 
                            ...
                            : msg.role === 'error'
                            ? 'bg-red-900/30 text-red-300 rounded-bl-none border border-red-700/50'
                  ...
                  <form onSubmit={handleChat} className="p-3 border-t border-gray-800 bg-[#1a1f2e] rounded-b-xl flex flex-col gap-2">
                    <div className="flex flex-wrap gap-1.5 mb-1 px-1">
                      {['Analyze latest news', 'What are the key resistance levels?', 'Summarize fundamentals', 'Analyze recent earnings'].map((prompt, i) => (
                        <button
                          key={i}
                          type="button"
                          onClick={() => setChatInput(prompt)}
                          className="text-[10px] leading-tight bg-[#232936] hover:bg-[#2d3545] text-gray-300 px-2 py-1.5 rounded-full border border-gray-700 transition-colors text-left"
                        >
                          {prompt}
                        </button>
                      ))}
