...
class ChatAgent:
    ...
    def explain(
        ...
        # 3. Earnings Query
        if "earning" in q or "transcript" in q or "results" in q:
             # Look for earnings specifically or use LLM to summarize
             return (
                 f"When analyzing earnings for {ticker}, the key factors to watch are revenue growth, forward guidance, and CEO commentary on margins.\n\n"
                 f"Recent news highlights:\n"
                 f"{chr(10).join(['- ' + n['title'] for n in news_items[:3]]) if news_items else '- No recent earnings news found.'}\n\n"
                 f"From a fundamental standpoint, {ticker} shows a Profit Growth (3Y) of {fundamentals.metrics.get('profit_growth_3y', 'N/A')}%. "
                 f"If you'd like a deep dive transcript summary, please upload the specific PDF/text."
             )

        # 4. Technical/Support/Levels Query
