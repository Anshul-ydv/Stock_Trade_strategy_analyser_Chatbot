...
def score_for_strategy(
    ...
    if strategy == "swing":
        ...
        # VCP favors tightening
        if 40 < rsi < 70: ta_score += 10
        if macd > 0: ta_score += 5
    elif strategy in ["sma_crossover", "macd_cross"]:
        # Basic momentum strategies
        if macd > macd_sig: ta_score += 15
    ...
    if strategy == "breakout":
        ...
        # VCP breakout entry
        entry = close
        stop = close - 1.0 * atr # Tighter stop for VCP
        target = close + 3.0 * atr
    elif strategy in ["sma_crossover", "macd_cross"]:
        entry = close
        stop = float(df["ema_50"].iloc[-1]) if "ema_50" in df else close - 2.0 * atr
...
def _score_from_signals(strategy: str, signals: Iterable[Signal]) -> float:
    relevant_indicator_map = {
        "breakout": {"breakout", "double_top", "double_bottom"},
        "vcp": {"vcp", "breakout"},
        "sma_crossover": {"sma_crossover"},
        "macd_cross": {"macd_cross"},
        "swing": {"double_bottom", "macd_cross", "sma_crossover"}
    }
    
    target_indicators = relevant_indicator_map.get(strategy, set())
    relevant = [s for s in signals if s.indicator in target_indicators]
    
    if not relevant:
        return 40.0
    best = max(relevant, key=lambda s: s.score)
    bonus = 10 if strategy == best.indicator else 0 # Bonus for exact match
...
def rank_strategies(
    ...
    strategies = list(strategies or ["breakout", "swing", "intraday", "vcp", "sma_crossover", "macd_cross"])
    results = [
