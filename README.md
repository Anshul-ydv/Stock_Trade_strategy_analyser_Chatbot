...
cd Trading_Chatbot/trading-chatbot
```
...
Key settings:
- **LLM_PROVIDER**: Set to `ollama` for local LLM or `openai` for OpenAI API
- **FYERS_CLIENT_ID / FYERS_ACCESS_TOKEN**: Optional, for live market data
- **ALLOW_SYNTHETIC_DATA**: Set to `1` to use generated data when APIs are unavailable

### 5. Fyers (Optional)

Use the `authtoken.ipynb` notebook to generate the Fyers access token.
Refer to: https://myapi.fyers.in
...
From the `trading-chatbot` directory:
